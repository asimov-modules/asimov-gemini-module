...
#[cfg(not(feature = "std"))]
compile_error!("asimov-gemini-prompter requires the 'std' feature");
...
pub fn main() -> Result<SysexitsError, Box<dyn Error>> {
    ...
    let Ok(manifest) = asimov_module::ModuleManifest::read_manifest("gemini")
    ...
    let options = asimov_gemini_module::Options::builder()
        .endpoint(endpoint)
        .model(model)
        .api_key(api_key)
        .build();

    let response = asimov_gemini_module::generate(&input, &options)?;
